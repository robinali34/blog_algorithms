---
layout: page
title: About
permalink: /about/
---

## About

Welcome to my Graduate Algorithms blog!

This blog focuses on Algorithms course notes, study guides, and algorithm resources. Here you'll find comprehensive notes, problem-solving strategies, and study materials for the Graduate Algorithms course.

## What You'll Find Here

- **Course Notes**: Detailed notes from lectures and course materials
- **Problem Solutions**: Step-by-step solutions to practice problems
- **Study Guides**: Comprehensive study guides for exams and assignments
- **Algorithm Resources**: Additional resources and references for algorithms covered in the course

## Contact

Feel free to reach out if you have any questions or would like to discuss career topics!

### Get in Touch

- **GitHub**: [@robinali34](https://github.com/robinali34)
- **LinkedIn**: Connect with me for professional discussions
- **Email**: [robinali34@gmail.com](mailto:robinali34@gmail.com)

### What You'll Find Here

- Dynamic programming and optimization problems
- Graph algorithms and network flows
- NP-completeness and computational complexity
- Algorithm design and analysis techniques

### Response Time

I typically respond to messages within 24-48 hours. For urgent matters, please mention it in your message.


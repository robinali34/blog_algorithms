...
**Rudrata Path:**
- Path: $1 \to 4 \to 5 \to 2 \to 3 \to 6$ ✓ (visits all 6 vertices exactly once)
- Path: $1 \to 2 \to 5 \to 4$ ✗ (doesn't visit all vertices)
- Path: $1 \to 2 \to 3 \to 2$ ✗ (visits vertex 2 twice)

**Rudrata (1,6)-Path:**
- Path: $1 \to 4 \to 5 \to 2 \to 3 \to 6$ ✓ (starts at 1, ends at 6, visits all vertices)
- Path: $1 \to 2 \to 5 \to 4 \to 3 \to 6$ ✓ (another valid (1,6)-path)
...
- Rudrata path: $1 \to 2 \to 4 \to 3$ (visits all 4 vertices)
- Note: This graph also has a Hamiltonian cycle: $1 \to 2 \to 4 \to 3 \to 1$
...
2. Pick an arbitrary edge $(u, v) \in E$
...
- If $G'$ has a Rudrata (s,t)-path, it must go $s \to u \to \ldots \to v \to t$
...
1. **For each variable $x_i$:**
   - Create a "variable gadget": a path with vertices representing choosing $x_i = \text{TRUE}$ or $x_i = \text{FALSE}$
   - Typically: vertices $v_{i,1}, v_{i,2}, \ldots, v_{i,k}$ where the path can go "left" (TRUE) or "right" (FALSE)
...
- If $\phi$ is satisfiable, construct path through variable gadgets based on assignment
...
**Subproblem:** $dp[mask][v] = \text{true}$ if there exists a path visiting all vertices in $mask$ ending at vertex $v$.
...
- Recurrence: $dp[mask][v] = \bigvee_{u \in mask, (u,v) \in E} dp[mask \setminus \{v\}][u]$
  - Path ending at $v$ visiting vertices in $mask$ exists if there's a neighbor $u$ such that path ending at $u$ visiting $mask \setminus \{v\}$ exists
...
- **Time Complexity:** $O(2^n \cdot n^2)$
- **Space Complexity:** $O(2^n \cdot n)$ (can be optimized with careful implementation)
...
5. **Time complexity analysis**: For the DP algorithm, verify the $O(2^n \cdot n^2)$ time complexity. What is the space complexity?

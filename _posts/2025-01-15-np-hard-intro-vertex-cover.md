...
Since $k \leq |V|$ and we need to check $|E|$ edges, this verification takes polynomial time in the input size. Therefore, Vertex Cover is in NP.
...
- This follows directly from Gallai's theorem:
  - If maximum independent set has size $\geq k$, then minimum vertex cover has size $\leq |V| - k$
  - If minimum vertex cover has size $\leq |V| - k$, then maximum independent set has size $\geq k$
...
1. **Create vertices**: 
   - For each variable $x_i$, create two vertices: one for $x_i$ and one for $\neg x_i$
...
- Round: include vertex $v$ if $x_v \geq 1/2$
...
**Algorithm:** Check all subsets of vertices of size $\leq k$
...
- **Recurrence:** $dp[mask] = \min_{v \notin mask} \{dp[mask \cup \{v\}] + 1\}$
...
- **Recurrence:**
  - $dp[v][0] = \sum_{u \in children(v)} dp[u][1]$ (must cover edges to children)
...
- **Time Complexity:** $O(\text{LP solver time}) = O(n^{3.5}L)$ using interior-point methods
- **Space Complexity:** $O(n + |E|)$
- **Approximation Ratio:** 2

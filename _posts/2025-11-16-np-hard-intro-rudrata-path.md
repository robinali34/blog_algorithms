...
1. Check that the path has exactly n vertices: O(n) time
2. Check that each vertex appears exactly once: O(n) time (use a set or array)
3. Check that consecutive vertices in the path are adjacent: O(n) time (check n-1 edges)

Total verification time: O(n), which is polynomial in the input size. Therefore, Rudrata Path is in NP.
...
**Polynomial Time:**
- The reduction takes O(n + m) time
